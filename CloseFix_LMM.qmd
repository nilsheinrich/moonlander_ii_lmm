...
```
Results:
**N_visible_obstacles** (0.000918353, 0.0267634)
**N_visible_drift_tiles** is significant, increasing fixation duration (0.0155779, 0.164635).
...
```
m_varyingSlope_complex_ni is slightly favored by BIC (not by AIC though). We will proceed with zercorr from here on.
...
```
m_varyingSlope_complex_ni is favored by BIC and block should therefore be kept as random slope.
...
```
Again, m_varyingSlope_complex_ni is favored.
...
```
m_varyingSlope3 has lower BIC than m_varyingSlope_complex_ni. Proceeding with m_varyingSlope3.

Trying to delete even more? Starting with block
```{julia}
#| label: m_varyingSlope4
...
issingular(m_varyingSlope4) # NOT overparameterized
...
gof_summary = let
  nms = [:m_varyingSlope4, :m_varyingSlope3]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope4, m_varyingSlope3)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
Nope. That increased BIC again.

For the sake of completion, deleting N_visible_obstacles instead:
```{julia}
#| label: m_varyingSlope5
...
issingular(m_varyingSlope5) # NOT overparameterized
...
gof_summary = let
  nms = [:m_varyingSlope5, :m_varyingSlope3]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope5, m_varyingSlope3)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
Again a worse fit than before. Sticking to m_varyingSlope3.
...
We will omit the intercept because it would zoom out too much.
```{julia}
ridgeplot(samples; show_intercept=false, xlabel="Bootstrap density and 95%CI", title="Distance to agent (close fixations)")
```
Significant effects:
**N_visible_obstacles** (0.00622244, 0.0207252), which increases the distance to the spaceship in close fixations with increasing N of obstacles on screen.

**N_visible_drift_tiles** and **block: fake** interact positively with each other (0.0112474, 0.059733), as do...
...**N_visible_drift_tiles** and **block: invisible** (0.00924491, 0.0570686).
No main effects for either block type though.
