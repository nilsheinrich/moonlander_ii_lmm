...
Throwing drift out of the random effects structure of the model.
```{julia}
...
```
Of the fixed effects included in our model, only the interaction effect of **N_visible_drift_tiles** is significant, increasing fixation duration (0.0200105, 0.168487).
None of **drift** comparisons, nor the the interaction **N_visible_obstaclesN_visible_obstacles** is significant.
...
m_varyingInt1 = let
    varInt = @formula(distance_to_spaceship ~ 1 + N_visible_obstacles + N_visible_drift_tiles * drift + 
    (1 | ID));
    fit(MixedModel, varInt, my_data; contrasts=my_cake);
  end
...
m_varyingSlope_complex = let
    varSlope = @formula(distance_to_spaceship ~ 1 + N_visible_obstacles + N_visible_drift_tiles * drift + 
    (1 + N_visible_obstacles + N_visible_drift_tiles * drift | ID));
...
```
_complex_zc is favored by BIC and drift should therefore be kept as random slope.
...
ridgeplot(samples; show_intercept=false, xlabel="Bootstrap density and 95%CI")
```
We detect a significant effect for **N_visible_obstacles** (0.00970436, 0.0422942), which increases the distance to the spaceship in resting fixations with increasing N of obstacles on screen.
